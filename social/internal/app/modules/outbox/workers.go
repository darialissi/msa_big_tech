...
type (
	// FriendRequestEventHandler - обработчик событий по заявкам
	FriendRequestEventHandler interface {
...
// OutboxFriendRequestWorker — обработка outbox-событий именно по заявкам
type OutboxFriendRequestWorker struct {
	OutboxWorker

	repo    Repository
	tm      TransactionManager
	handler FriendRequestEventHandler
}
...
func (w *OutboxFriendRequestWorker) Fetch(ctx context.Context) error {
	...
	events := w.repo.SearchEvents(
		...
		// 3-я ступень pruning
		WithEventType(EventTypeFriendRequestCreated),
		// фильтрация
		WithOnlyUnpublished(),
		WithDueAt(now),
	...
	if len(succeeded) > 0 {
		e := w.repo.UpdateEvents(
			...
			WithUpdateEventType(EventTypeFriendRequestCreated),
